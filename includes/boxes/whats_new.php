...
$product_query = tep_db_query("select products_description, products_id from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$random_product['products_id'] . "' and language_id = '" . (int)$languages_id . "'");
    ...
    if (tep_not_null($random_product['specials_new_products_price'])) {
	  ...
	  $p_details = '<a href="' . tep_href_link('product_info.php?products_id='.$random_product['products_id']) . '">'.tep_image_button('button_details2.gif', '', '').'</a>';
	  $p_buy_now = '<a href="'.tep_href_link("products_new.php","action=buy_now&products_id=".$random_product['products_id']).'">'.tep_image_button('button_add_to_cart2.gif', '', '').'</a>';	
    } else {
	  ...
	  $p_details = '<a href="' . tep_href_link('product_info.php?products_id='.$random_product['products_id']) . '">'.tep_image_button('button_details4.gif', '', '').'</a>';
	  $p_buy_now = '<a href="'.tep_href_link("products_new.php","action=buy_now&products_id=".$random_product['products_id']).'">'.tep_image_button('button_add_to_cart4.gif', '', '').'</a>';	
    }
